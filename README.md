...
2. Указать свои настройки в `.env`:
   Пути не должны содержать обратного слеша в конце пути `/`
   Путь до директории должен существовать, не успел залоггировать или вынести в отдельную функцию создание папки
   Поэтому в случае, если сервис не обрабатывает директорию, то нужно проверить путь до директории (o/i).
